...
MSHDEF void    msh_distrib2pdf( const double* dist, double* pdf, int32_t n_vals );
MSHDEF void    msh_pdf2cdf( const double* pdf, double* cdf, int32_t n_vals );
MSHDEF void    msh_invert_cdf( const double* cdf, int32_t n_vals,
                              double* invcdf, int32_t n_invcdf_bins );
MSHDEF int     msh_pdfsample_linear( const double* pdf, double prob, int32_t n_vals);
MSHDEF int     msh_pdfsample_invcdf( const double* pdf, double prob, int32_t n_vals);
...
MSHDEF void msh_discrete_distribution_init(msh_discrete_distrib_t* ctx,
                                           double* weights, int32_t n_weights, uint32_t seed );
...
MSHDEF void
msh_invert_cdf( const double* cdf, int32_t n_vals, double* invcdf, int32_t n_invcdf_bins )
{
    int32_t prev_x = 0;
    for( int32_t i = 0 ; i < n_vals; ++i )
    {
        int32_t cur_x = (int32_t)(cdf[i] * (n_invcdf_bins-1));
        for( int32_t x = prev_x; x <= cur_x; ++x )
        {
            invcdf[ x ] = i;
        }
        prev_x = cur_x;
