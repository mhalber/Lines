...
tex_buffer_lines_init_device()
{
...
#if USE_BINDLESS_TEXTURES
      layout(location = 1) uniform uint64_t u_line_data_handle;
#else
      layout(location = 1) uniform samplerBuffer u_line_data_sampler;
#endif
  ...
  glTextureBuffer( device->line_data_texture_id, GL_RGB32F, device->line_data_buffer );
